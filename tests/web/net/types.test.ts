...
import type { CreateExpenseSchema } from '../../../src/server/api/schemas.js'
...
const createExpenseMatches: Exact<
...
describe('wire type contract', () => {
  it('keeps src/web/net/types.ts in sync with the server (enforced by npm run typecheck)', () => {
    expect(
      [groupMatches, detailsMatch, settlementMatches, createExpenseMatches].every(
        Boolean,
      ),
    ).toBe(true)
  })
