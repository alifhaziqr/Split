...
import { GroupNotFoundError, GroupNotEmptyError } from '../../../src/server/db/groups.js'
import { DuplicateMemberError, MemberReferencedError, MemberNotFoundError } from '../../../src/server/db/members.js'
import { ExpenseNotFoundError, MemberNotInGroupError } from '../../../src/server/db/expenses.js'
...
describe('toErrorResponse', () => {
  ...
  it('never echoes a SyntaxError instance\'s own message into the response body', () => {
    ...
    const syntaxError = new SyntaxError('/home/afrina/split/dev.db: internal detail that must never reach a client')
  ...
  it('maps core ValidationError to 422 INVALID_SPLIT', () => {
    const { status, body } = toErrorResponse(new ValidationError('shares do not sum to amount'))
  ...
  it('maps an unrecognized plain Error to 500 INTERNAL_ERROR without leaking its message', () => {
    ...
    const { status, body } = toErrorResponse(new Error(`could not open database file at ${secret}`))
  ...
  it('maps a plain Error styled like settle.ts corrupted-data messages to 500, not 422', () => {
    const { status, body } = toErrorResponse(new Error('Balances must sum to zero, got 5'))
    expect(status).toBe(500)
    expect(body.error.code).toBe('INTERNAL_ERROR')
  })
