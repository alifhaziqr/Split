        ...
        <li key={balance.memberId} className="space-y-1">
          ...
          >
            {balance.balanceCents > 0 && (
              <>
                <span>{nameFor(balance.memberId)} is owed</span>{' '}
                <Money cents={balance.balanceCents} currency={props.currency} />
              </>
            )}
            {balance.balanceCents < 0 && (
              <>
                <span>{nameFor(balance.memberId)} owes</span>{' '}
                <Money cents={-balance.balanceCents} currency={props.currency} />
              </>
            )}
            {balance.balanceCents === 0 && (
              <span>{nameFor(balance.memberId)} is settled up</span>
            )}
          </div>
